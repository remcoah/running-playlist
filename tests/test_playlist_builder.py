...
def _song(path, bpm, energy, duration_secs=200):
    return {"path": path, "bpm": bpm, "duration_secs": duration_secs, "energy": energy, "last_played": None}
...
class TestPickSongBpmProximity:
    def test_bpm_closest_candidate_wins_over_energy_closest_but_farther_in_bpm(self):
        ...
        bpm_close = _song("bpm_close.mp3", bpm=165, energy=0.95)         # distance 0, mediocre energy
        decoy_1 = _song("decoy1.mp3", bpm=166, energy=0.99)              # distance 1
        decoy_2 = _song("decoy2.mp3", bpm=164, energy=0.99)              # distance 1
        energy_close_bpm_far = _song("energy_close.mp3", bpm=155, energy=0.50)  # distance 10, exact energy

        winner = _pick_song(
            [bpm_close, decoy_1, decoy_2, energy_close_bpm_far], slot_bpm, slot_energy,
            tolerance=15, used_paths=[],
        )
    ...
    def test_shortlist_excludes_candidates_beyond_the_proximity_window(self):
        ...
        near_2 = _song("near2.mp3", bpm=163, energy=0.20)   # distance 2, best energy match within top 3
        near_3 = _song("near3.mp3", bpm=170, energy=0.05)   # distance 5
        far_perfect_energy = _song("far.mp3", bpm=178, energy=0.50)  # distance 13 -> 4th closest, excluded

        winner = _pick_song(
            [far_perfect_energy, near_1, near_2, near_3], slot_bpm, slot_energy,
            tolerance=15, used_paths=[],
        )
    ...
    def test_half_time_match_is_sorted_by_half_time_distance_not_raw_distance(self):
        ...
        half_time_perfect = _song("ht_perfect.mp3", bpm=81, energy=0.50)  # half-time distance 1, exact energy
        ...
        direct_c = _song("direct_c.mp3", bpm=170, energy=0.20)  # direct distance 10 -> should be excluded

        winner = _pick_song(
            [half_time_perfect, direct_a, direct_b, direct_c], slot_bpm, slot_energy,
            tolerance=10, used_paths=[],
        )
